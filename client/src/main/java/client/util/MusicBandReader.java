...
/**
 * Сборка {@link MusicBand} из консоли (без полей цены/валюты).
 */
public final class MusicBandReader {
    ...
    public static MusicBand readForUpdate(InputHandler input, MusicBand current) {
        ...
        String participantsStr = input.rawScan("Количество участников ["
                + (current.getNumberOfParticipants() == null ? "не указано" : current.getNumberOfParticipants())
                + "] (введите 'null' для соответ. значения)");
        ...
        if (participantsStr == null || participantsStr.isEmpty()) {
            ...
            participants = InputHandler.confirmNull(input) ? null : current.getNumberOfParticipants();
        } else {
            try {
        ...
        String genreStr = input.rawScan("Жанр [" + (current.getGenre() == null ? "не указан" : current.getGenre())
                + "] (введите 'null' для соответ. значения)");
        if (genreStr == null || genreStr.isEmpty()) {
            band.setGenre(current.getGenre());
        } else if (genreStr.equalsIgnoreCase("null")) {
            if (InputHandler.confirmNull(input)) {
                band.setGenre(null);
            } else {
                band.setGenre(current.getGenre());
            }
        ...
        String frontManName = input.rawScan("Имя фронтмена ["
                + (current.getFrontMan() == null ? "не указано" : current.getFrontMan().getName())
                + "] (введите 'null' для соответ. значения, или повторите значение для обновления полей пользователя)");
        if (frontManName == null || frontManName.isEmpty()) {
