...
public final class PersonReader {
    ...
    public static Person readForUpdate(InputHandler input, Person current, String nameParam) {
        ...
        String birthdayRaw = input.rawScan(String.format(
                "Введите дату рождения фронтмена [%s] (формат yyyy-MM-dd HH:mm:ss) (введите 'null' для соответ. значения)",
                currB));
        ...
        String heightRaw = input.rawScan(String.format(
                "Введите рост фронтмена [%s] (или Enter для пропуска) (введите 'null' для соответ. значения)", currH));
