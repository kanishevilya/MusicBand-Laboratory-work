...
/**
 * Команда replace_if_lower для замены элемента коллекции по ID
 */
public class ReplaceLowerCommand implements Command {
    private final CollectionManager collectionManager;
    private final CommandManager commandManager;

    public ReplaceLowerCommand(CollectionManager collectionManager, CommandManager commandManager) {
    ...
    public void execute(String[] args) {
        if (args.length < 2) {
            System.out.println("Ошибка: укажите ключ. Использование: replace_if_lower <ключ>");
            return;
        }
        long key;
        try {
